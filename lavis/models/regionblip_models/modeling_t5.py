...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.q = LoraLinear(self.d_model, self.inner_dim, bias=False, r=8).float()
        self.k = nn.Linear(self.d_model, self.inner_dim, bias=False)
        #self.v = nn.Linear(self.d_model, self.inner_dim, bias=False)
        self.v = LoraLinear(self.d_model, self.inner_dim, bias=False, r=8).float()
    ...
    def forward(
        ...
        # get query states
        query_states = shape(
            self.q(hidden_states, modal_name=modal_name)
        )  # (batch_size, n_heads, seq_length, dim_per_head)
        ...
        value_states = project(
            hidden_states,
            #self.v,
            partial(self.v, modal_name=modal_name),
            key_value_states,
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return tuple(module(*inputs, use_cache, output_attentions, modal_name=modal_name))
