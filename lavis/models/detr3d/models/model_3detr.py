...
from lavis.models.detr3d.third_party.pointnet2.pointnet2_modules import PointnetSAModuleVotes
from lavis.models.detr3d.third_party.pointnet2.pointnet2_utils import furthest_point_sample
from lavis.models.detr3d.utils.pc_util import scale_points, shift_scale_points

from lavis.models.detr3d.models.helpers import GenericMLP
from lavis.models.detr3d.models.position_embedding import PositionEmbeddingCoordsSine
from lavis.models.detr3d.models.transformer import (MaskedTransformerEncoder, TransformerDecoder,
                                TransformerDecoderLayer, TransformerEncoder,
                                TransformerEncoderLayer)
from lavis.models.detr3d.utils.box_util import (
...
class Model3DETR(nn.Module):
    ...
    def __init__(
        ...
        self.build_mlp_heads(decoder_dim, mlp_dropout, num_semcls, num_angle_bin)

        self.num_queries = num_queries
        #self.box_processor = BoxProcessor(dataset_config)

    def build_mlp_heads(self, decoder_dim, mlp_dropout, num_semcls, num_angle_bin):
        mlp_func = partial(
            GenericMLP,
            norm_fn_name="bn1d",
            activation="relu",
        ...
        angle_cls_head = mlp_func(output_dim=num_angle_bin)
        angle_reg_head = mlp_func(output_dim=num_angle_bin)
    ...
    def get_box_predictions(self, query_xyz, point_cloud_dims, box_features, box_processor):
        """
        Parameters:
        ...
        for l in range(num_layers):
            ...
            (
                center_normalized,
                center_unnormalized,
            ) = box_processor.compute_predicted_center(
                center_offset[l], query_xyz, point_cloud_dims
            )
            angle_continuous = box_processor.compute_predicted_angle(
                angle_logits[l], angle_residual[l]
            )
            size_unnormalized = box_processor.compute_predicted_size(
    ...
    def forward(self, inputs, encoder_only=False, box_processor=None):
        point_clouds = inputs["point_clouds"]                                       # [batch, 20000, 3]

        enc_xyz, enc_features, enc_inds = self.run_encoder(point_clouds)
        enc_features = self.encoder_to_decoder_projection(
            enc_features.permute(1, 2, 0)).permute(2, 0, 1)                         # [2048, batch, 256]
        # enc_xyz: [batch, 2048, 3]
        # enc_featrues: [2048, batch, 256]
        ...
        # query_xyz: [batch, 256, 3]
        # query_embed: [batch, 256, 256], batch x channel x npoint
        enc_pos = self.pos_embedding(enc_xyz, input_range=point_cloud_dims)         # [batch, 256, 2048]
        ...
        box_features = self.decoder(
            tgt, enc_features, query_pos=query_embed, pos=enc_pos)[0]               # [num_layers x num_queries x batch x channel]

        box_predictions = self.get_box_predictions(
            query_xyz, point_cloud_dims, box_features, box_processor
        )
        
        box_predictions['box_features'] = box_features
        return box_predictions


def build_preencoder(cfg):
    mlp_dims = [3 * int(cfg.use_color), 64, 128, cfg.enc_dim]
...
def build_3detr(cfg):
    ...
    model = Model3DETR(
        ...
        num_semcls=cfg.num_semcls,
        num_angle_bin=cfg.num_angle_bin
    )
    ...
    if os.path.isfile(cfg.pretrained):
        ...
        logging.info("detr3d: load from {}".format(cfg.pretrained))
        if len(missing_keys) > 0:
            logging.info('detr pretrain missing keys: {}'.format(missing_keys))
        if len(unexpected_keys) > 0:
            logging.info('detr pretrain unexpected keys: {}'.format(unexpected_keys))
    return model
