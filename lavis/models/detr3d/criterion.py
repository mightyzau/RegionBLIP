...
from lavis.models.detr3d.utils.box_util import generalized_box3d_iou
from lavis.models.detr3d.utils.dist import all_reduce_average
from lavis.models.detr3d.utils.misc import huber_loss
...
class SetCriterion(nn.Module):
    def __init__(self, matcher, loss_weight_dict, num_semcls, num_angle_bin):
        super().__init__()
        ...
        semcls_percls_weights = torch.ones(self.num_semcls + 1)
    ...
    def loss_sem_cls(self, outputs, targets, assignments):
        pred_logits = outputs["sem_cls_logits"]                                         # [batch, query, classes + 1]
        ...
        gt_box_label[assignments["proposal_matched_mask"].int() == 0] = (
            pred_logits.shape[-1] - 1)                                                  # [batch, qeury]
...
def build_criterion(cfg, num_semcls, num_angle_bin):
    matcher = Matcher(
    ...
    loss_weight_dict = {
        ...
        "loss_sem_cls_weight": cfg.loss_sem_cls_weight,
        "loss_no_object_weight": cfg.loss_no_object_weight,
        "loss_angle_cls_weight": cfg.loss_angle_cls_weight,
        "loss_angle_reg_weight": cfg.loss_angle_reg_weight,
        "loss_center_weight": cfg.loss_center_weight,
        "loss_size_weight": cfg.loss_size_weight,
    }
    criterion = SetCriterion(matcher, loss_weight_dict, num_semcls, num_angle_bin)
    return criterion
