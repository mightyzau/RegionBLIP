...
class CLIP(BaseModel):
    ...
    def predict(self, samples):
        ...
        image = samples["image"]                                                    # [batch, 3, 336, 336]
        targets = samples["label"]                                                  # [batch,]

        image_features = self.encode_image(image)
        image_features = F.normalize(image_features, dim=-1)                        # [batch, 768]
    ...
    def before_evaluation(self, dataset, task_type, **kwargs):
        # task_type: <class 'lavis.tasks.multimodal_classification.MultimodalClassificationTask'>

        if task_type == MultimodalClassificationTask:
            self.classifier = self.zero_shot_classifier(
                classnames=dataset.classnames,              # list of 1000 category names
    ...
    def zero_shot_classifier(self, classnames, templates):
        with torch.no_grad():
            ...
            for classname in classnames:
                ...
                texts = self.tokenizer(texts).to(self.device)  # tokenize,                  (#templates, 77)

                class_embeddings = self.encode_text(texts)                                  # (#templates, 768)
                class_embedding = F.normalize(class_embeddings, dim=-1).mean(dim=0)         # (768,)
                class_embedding /= class_embedding.norm()
                zeroshot_weights.append(class_embedding)
            zeroshot_weights = torch.stack(zeroshot_weights, dim=1).to(self.device)         # (768, #category)
    ...
    def compute_sim_matrix(self, data_loader, **kwargs):
        ...
        num_text = len(texts)                   # 25010 for coco_retrieval
        text_bs = 256
        text_features = []
        ...
        for i in range(0, num_text, text_bs):
            ...
            text_input = self.tokenizer(text).to(self.device)   # [text_bs, 77]

            text_feat = self.encode_text(text_input)            # [text_bs, 768]
            text_feat = F.normalize(text_feat, dim=-1)

            text_features.append(text_feat)

        text_features = torch.cat(text_features, dim=0)         # [num_text, 768]

        image_features = []
        for samples in data_loader:
            image = samples["image"]

            image = image.to(self.device)                       # [batch, 3, 336, 336]
            image_feat = self.encode_image(image)               # [batch, 768]
