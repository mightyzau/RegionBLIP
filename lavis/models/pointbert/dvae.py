...
from lavis.models.pointbert import misc

try:
    from knn_cuda import KNN
except:
    import sys 
    sys.path.insert(0, "/mnt_jianchong/jianchong/projects_A100/LLM-multimodal/KNN_CUDA/build/lib")
    from knn_cuda import KNN
...
def knn_point(nsample, xyz, new_xyz):
    ...
    """
    sqrdists = square_distance(new_xyz, xyz)                                                # [batch, S, N]
    _, group_idx = torch.topk(sqrdists, nsample, dim=-1, largest=False, sorted=False)       # [batch, S, nsample]
    return group_idx
...
class Group(nn.Module):
    ...
    def forward(self, xyz):
        ...
        # fps the centers out
        center = misc.fps(xyz, self.num_group)                  # [batch, num_group, 3]
        # knn to get the neighborhood
        idx = knn_point(self.group_size, xyz, center)           # [batch, num_group, group_size]
        ...
        neighborhood = neighborhood.view(batch_size, self.num_group, self.group_size, 3).contiguous()   # [batch, num_group, group_size, 3]
        # normalize
        neighborhood = neighborhood - center.unsqueeze(2)                                               # [batch, num_group, group_size, 3]

        if colors is not None:
            colors = colors.view(batch_size * num_points, -1)[idx, :]
            colors = colors.view(batch_size, self.num_group, self.group_size, 3).contiguous()
            neighborhood = torch.cat([neighborhood, colors], dim=-1)
...
class Encoder(nn.Module):
    def __init__(self, encoder_channel):
        super().__init__()
        self.encoder_channel = encoder_channel      # 256
    ...
    def forward(self, point_groups):
        '''
            point_groups : [batch, num_group, group_size, 3], B G N 3
            -----------------
            feature_global : B G C
        '''
        bs, g, n, _ = point_groups.shape
        point_groups = point_groups.reshape(bs * g, n, 3)                           # [batch*num_group, group_size, 3]
        # encoder
        feature = self.first_conv(point_groups.transpose(2, 1))                     # [batch*num_group, 256, group_size]
        feature_global = torch.max(feature, dim=2, keepdim=True)[0]                 # [batch*num_group, 256, 1]

        feature = torch.cat([feature_global.expand(-1, -1, n), feature], dim=1)     # [batch*num_group, 512, group_size]
        feature = self.second_conv(feature)                                         # [batch*num_group, 256, group_size]
        feature_global = torch.max(feature, dim=2, keepdim=False)[0]                # [batch*num_group, 256]
        return feature_global.reshape(bs, g, self.encoder_channel)

class EncoderWithColor(nn.Module):
    def __init__(self, encoder_channel):
        super().__init__()
        self.encoder_channel = encoder_channel      # 256
        self.first_conv = nn.Sequential(
    ...
    def forward(self, point_groups):
        ...
        point_groups = point_groups.reshape(bs * g, n, c)                           # [batch*num_group, group_size, 3]
        # encoder
        feature = self.first_conv(point_groups.transpose(2, 1))                     # [batch*num_group, 256, group_size]
        feature_global = torch.max(feature, dim=2, keepdim=True)[0]                 # [batch*num_group, 256, 1]
