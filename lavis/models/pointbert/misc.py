...
def fps(xyz, npoint):
    ...
    device = xyz.device
    B, N, C = xyz.shape
    centroids = torch.zeros(B, npoint, dtype=torch.long).to(device)             # [batch, npoint]
    distance = torch.ones(B, N).to(device) * 1e10                               # [batch, N]
    farthest = torch.randint(0, N, (B,), dtype=torch.long).to(device)           # [batch,]
    batch_indices = torch.arange(B, dtype=torch.long).to(device)                # [batch,]
    for i in range(npoint):
        centroids[:, i] = farthest
        centroid = xyz[batch_indices, farthest, :].view(B, 1, 3)                # [batch, 1, 3]
        dist = torch.sum((xyz - centroid) ** 2, -1)                             # [batch, N]
